...
NS_LOG_COMPONENT_DEFINE("ndn.FWWProducer");
...
Producer::OnInterest(shared_ptr<const Interest> interest)
{
  ...
  if (!m_active)
    return;
  //fww 打印相关的兴趣包信息
  //std::string pushPrefix = "/ndn/push/data/";
  //int node = ns3::Simulator::GetContext();
  //std::string incomingPrefix = interest->getName().toUri (); 
  //std::cout<<"node-"<<node<<"-receive interest====>"<<incomingPrefix<<std::endl;
  ...
  int node = ns3::Simulator::GetContext();
  data->setBetw(2018);  
  data->setTag(std::make_shared<ndn::lp::FwwTag>(node));
